...
public:
   ...
   */
  void
  calculateLID (vector<Bitvector> &LIDs, int index);
   ...
   */
  void
  discoverMacAddresses (bool no_remote);
   ...
   */
  string
  getTestbedIPFromLabel (string label);
   ...
   */
  void
  writeClickFiles (bool montoolstub, bool dump_supp);
  void
  writeNS3ClickFiles ();
   ...
   */
  void
  scpTMConfiguration (string name);
   ...
   */
  void
  assignDeviceNamesAndMacAddresses ();
  NetworkNode *
  getNode (string label);
  string
  getNextMacAddress ();
  void
  createNS3SimulationCode ();
};
