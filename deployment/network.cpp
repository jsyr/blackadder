...
void
Domain::calculateLID (vector<Bitvector> &LIDs, int index)
{
  int bit_position;
...
string
Domain::getTestbedIPFromLabel (string label)
{
...
void
Domain::discoverMacAddresses (bool no_remote)
{
  ...
  for (size_t i = 0; i < network_nodes.size (); i++)
  {
    ...
    {
      NetworkConnection *nc = nn->connections[j];
      if (overlay_mode.compare ("mac") == 0 || overlay_mode.compare ("mac_ml") == 0 || overlay_mode.compare ("mac_qos") == 0)
      {
	if (overlay_mode.compare ("mac_ml") == 0 && (nn->type != "PN" || nc->src_if == ""))
	{
	  continue;
	}
	...
	if (overlay_mode.compare ("mac_ml") == 0 && (nn->type != "PN" || nc->dst_if == ""))
	{
	  continue;
	}
...
NetworkConnection *
NetworkNode::getConnection (string src_label, string dst_label)
{
...
void
Domain::assignDeviceNamesAndMacAddresses ()
{
  for (size_t i = 0; i < network_nodes.size (); i++)
  {
    NetworkNode *nn = network_nodes[i];
...
void
Domain::createNS3SimulationCode ()
{
  ofstream ns3_code;
...
void
Domain::writeClickFiles (bool montoolstub, bool dump_supp)
{
  ofstream click_conf;
  ...
  for (size_t i = 0; i < network_nodes.size (); i++)
  {
    ...
    for (size_t j = 0; j < nn->connections.size (); j++)
    {
      NetworkConnection *nc = nn->connections[j];
      int offset;
      if (overlay_mode.compare ("mac") == 0 || (overlay_mode.compare ("mac_ml") == 0 && nn->type.compare ("PN") == 0))
      {
	if (overlay_mode.compare ("mac_ml") == 0 && nc->lnk_type != "pp")
	{
	...
	for (size_t i = 0; i < it->second.size (); i++)
	{
	  ...
	  click_conf << "fw[" << it->second[i].fwport << "] -> inC_" << ifpname << " -> " << " tsQ_" << ifpname << " -> BS_" << it->first << "_" << ((i + 1)/* *it_count*/) << " -> outC_" << ifpname
	      << " -> [" << i << "]sc_" << it->first << ";" << endl;
	}
...
void
Domain::scpTMConfiguration (string TM_conf)
{
  FILE *scp_command;
  string command;
...
void
Domain::scpClickBinary (string tgzfile)
{
  string command;
  for (size_t i = 0; i < network_nodes.size (); i++)
  {
...
string
Domain::writeConfigFile (string filename)
{
