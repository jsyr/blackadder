...
int
Parser::getGlobalDomainParameters ()
{
  try
  {
...
int
Parser::getNS3GlobalDomainParameters ()
{
  try
  {
    dm->ba_id_len = cfg.lookup ("BLACKADDER_ID_LENGTH");
...
int
Parser::addConnection (const Setting &connection, NetworkNode *nn)
{
...
int
Parser::addNS3Connection (const Setting &connection, NetworkNode *nn)
{
  NetworkConnection *nc = new NetworkConnection ();
  nc->src_label = nn->label;
  /***********Parse the destination label**************/
...
int
Parser::addNS3Application (const Setting &application, NetworkNode *nn)
{
  string attributes;
...
int
Parser::addNode (const Setting &node)
{
  int ret;
  ...
  if (node.lookupValue ("operating_system", operating_system))
  {
    ...
    if ((operating_system.compare ("FreeBSD") == 0 || operating_system.compare ("Darwin") == 0) && (dm->overlay_mode.compare ("mac") != 0))
    {
...
int
Parser::addNS3Node (const Setting &node)
{
  int ret;
  NetworkNode *nn = new NetworkNode ();
  string node_label;
...
int
Parser::addPlanetLabNode (const Setting &node)
{
