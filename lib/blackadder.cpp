...
int Blackadder::create_and_send_buffers(unsigned char type, const string &id, const string &prefix_id, char strategy, void *str_opt, unsigned int str_opt_len) {
    ...
    nlh->nlmsg_len = sizeof (struct nlmsghdr) + sizeof (protocol) + sizeof (pid) + sizeof (type) + sizeof (id_len) + id.length() + sizeof (prefix_id_len) + prefix_id.length() + sizeof (strategy) + sizeof (str_opt_len) + str_opt_len;
    nlh->nlmsg_pid = pid;
    nlh->nlmsg_flags = 1;
    nlh->nlmsg_type = 0;
    iov[0].iov_base = nlh;
