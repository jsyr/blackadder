...
void NB_Blackadder::push(unsigned char type, const string &id, const string &prefix_id, char strategy, void *str_opt, unsigned int str_opt_len) {
    ...
    buffer_length = sizeof (struct nlmsghdr) + sizeof (protocol) + sizeof (pid) + sizeof (type) + sizeof (id_len) + id.length() + sizeof (prefix_id_len) + prefix_id.length() + sizeof (strategy) + sizeof (str_opt_len) + str_opt_len;
    ...
    memcpy(buffer + sizeof (struct nlmsghdr) + sizeof (protocol) + sizeof (pid) + sizeof (type) + sizeof (id_len) + id.length() + sizeof (prefix_id_len) + prefix_id.length() + sizeof (strategy), &str_opt_len, sizeof (str_opt_len));
    memcpy(buffer + sizeof (struct nlmsghdr) + sizeof (protocol) + sizeof (pid) + sizeof (type) + sizeof (id_len) + id.length() + sizeof (prefix_id_len) + prefix_id.length() + sizeof (strategy) + sizeof (str_opt_len), str_opt, str_opt_len);
    memset(&msg, 0, sizeof (msg));
    iov->iov_base = buffer;
...
void NB_Blackadder::publish_data(const string &id, unsigned char strategy, void *str_opt, unsigned int str_opt_len, void *a_data, unsigned int data_len) {
    ...
    if (str_opt_len < 0) {
        cout << "NB_Blackadder Library: str_opt_len must be >= 0" << endl;
    } else if (id.length() % PURSUIT_ID_LEN != 0) {
        cout << "NB_Blackadder Library: Could not send  - wrong ID size" << endl;
    } else {
