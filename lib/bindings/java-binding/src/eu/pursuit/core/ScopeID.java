...
public class ScopeID {
	...
	public ByteIdentifier getSegment(int index){
		return this.list.get(index);
	}
	/**
	 * Similar to List:sublist, it returns a view of the portion of this list between the specified
	 * fromIndex, inclusive, and toIndex, exclusive. 
	 * @param fromIndex
	 * @param toIndex
	 * @return
	 */
	public ScopeID getSubscope(int fromIndex, int toIndex){
		return new ScopeID(list.subList(fromIndex, toIndex));
	}
	...
	public void concat(ScopeID scope) {
		for (ByteIdentifier b : scope.list) {
			addSegment(b);
		}
	}
	
	public ScopeID deepCopy() {
		ScopeID copyScope = new ScopeID();
		for (ByteIdentifier id : this.list) {
			copyScope.addSegment(id.deepCopy());
		}
		return copyScope;
	}
