...
Scope::Scope(unsigned char _strategy, Scope *_father_scope) {
    if (_father_scope == NULL) {
        ...
        strategy = _strategy;
        isRoot = false;
        fatherScopes.find_insert(_father_scope);
        _father_scope->childrenScopes.find_insert(this);
    }
    str_opt_len = 0;
    str_opt = NULL;
}
...
void Scope::recursivelyUpdateIDs(ScopeHashMap &scopeIndex, IIHashMap &pubIndex, String suffixID) {
    ...
    for (ScopeSetIter child_it = childrenScopes.begin(); child_it != childrenScopes.end(); child_it++) {
        String tempSuffix = (*(*child_it).pointer->ids.begin()).first.substring((*(*child_it).pointer->ids.begin()).first.length() - PURSUIT_ID_LEN, PURSUIT_ID_LEN);
        (*child_it).pointer->recursivelyUpdateIDs(scopeIndex, pubIndex, tempSuffix);
    }
    InformationItemSet::iterator pub_it;
    for (pub_it = informationitems.begin(); pub_it != informationitems.end(); pub_it++) {
        String tempSuffix = (*(*pub_it).pointer->ids.begin()).first.substring((*(*pub_it).pointer->ids.begin()).first.length() - PURSUIT_ID_LEN, PURSUIT_ID_LEN);
        (*pub_it).pointer->updateIDs(pubIndex, tempSuffix);
    }
}
