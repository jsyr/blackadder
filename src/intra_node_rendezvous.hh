...
#ifndef CLICK_INTRANODERENDEZVOUS_HH
#define CLICK_INTRANODERENDEZVOUS_HH
...
class IntraNodeRendezvous : public RendezvousInterface {
public:
    IntraNodeRendezvous(RV *_rv_element);
    ~IntraNodeRendezvous();
...
private:
     ...
     * 
     */
    void notifySubscribers(unsigned char type, StringSet &IDs, unsigned char strategy, const void *str_opt, unsigned int str_opt_len, RemoteHostSet &subscribers);
    /**@brief This method will notify the Dispatcher to start or stop publishing data for an InformationItem for which has taken place.
     * 
     ...
     * The Dispatcher will then find the right ones for each interested application/publisher.
     ...
     * @param type
     */
    void notifyPublishers(InformationItem *pub, unsigned char type);
